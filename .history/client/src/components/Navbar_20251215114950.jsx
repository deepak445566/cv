...
function Navbar() {
  ...
  // ✅ Safe cart count
  const cartCount = getcount ? getcount() : 0;
    ...
    <>
      <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 py-2 border-b border-gray-300 bg-white relative transition-all z-40">
        ...
        <div className="hidden sm:flex items-center gap-8">
          ...
          <div onClick={() => navigate("/cart")} className="relative cursor-pointer hover:opacity-80">
            <img src={assets.nav_cart_icon} className='w-6 opacity-80' alt="Cart" />
            {/* ✅ Safe cart count display */}
            {cartCount > 0 && (
              <div className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full flex items-center justify-center">
                {cartCount}
              </div>
            )}
          </div>
            ...
            <div className='relative group'>
              ...
              <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-40 rounded-md text-sm z-40'>
                ...
                <li
                  ...
                  className='p-1.5 pl-3 hover:bg-primary/10 cursor-pointer text-red-600'
                >
                  Logout
                </li>
              </ul>
        ...
        <div className='flex items-center gap-6 sm:hidden'>
          <div onClick={() => navigate("/cart")} className="relative cursor-pointer">
            <img src={assets.nav_cart_icon} className='w-6 opacity-80' alt="Cart" />
            {/* ✅ Safe cart count display for mobile */}
            {cartCount > 0 && (
              <div className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full flex items-center justify-center">
                {cartCount}
              </div>
        ...
        <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
          <NavLink to="/" onClick={() => setOpen(false)} className="w-full py-2 hover:text-primary">
            Home
          </NavLink>
          <NavLink to="/products" onClick={() => setOpen(false)} className="w-full py-2 hover:text-primary">
            All Products
          </NavLink>
          {user && (
            <NavLink to="/myOrders" onClick={() => setOpen(false)} className="w-full py-2 hover:text-primary">
              My Orders
            </NavLink>
          )}
          <NavLink to="/" onClick={() => setOpen(false)} className="w-full py-2 hover:text-primary">
