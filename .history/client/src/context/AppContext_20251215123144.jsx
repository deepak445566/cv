// src/context/AppContext.js - UPDATED TOKEN HANDLING
...
export const AppContextProvider = ({ children }) => {
  ...
  // ✅ Create axios instance with BETTER token handling
  const createApi = () => {
    ...
    // ✅ FIXED: Get token from localStorage AND send in both ways
    api.interceptors.request.use((config) => {
      const token = localStorage.getItem("token");
      
      // Send token in Authorization header (for API routes)
      if (token) {
        config.headers.Authorization = `Bearer ${token}`;
      }
      
      // Also ensure cookies are sent
  ...
  const fetchUser = async () => {
    try {
      ...
      console.log("⚠️ Auth check failed, using cached data");
      // Don't clear on network error
    }
  };

  // ✅ FIXED: Login function - Save token properly
  const login = async (credentials) => {
    try {
      setLoading(true);
      console.log("Logging in with:", credentials.email);
      
      // Use a temporary axios instance without interceptors for login
